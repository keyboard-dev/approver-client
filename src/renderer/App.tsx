...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if ((window.electronAPI as unknown as { onConnectionStatusChange?: (handler: typeof handleConnectionStatusChange) => void }).onConnectionStatusChange) {
      (window.electronAPI as unknown as { onConnectionStatusChange: (handler: typeof handleConnectionStatusChange) => void }).onConnectionStatusChange(handleConnectionStatusChange)
    }
    ...
    return () => {
      ...
      if ((window.electronAPI as unknown as { removeAllListeners?: (event: string) => void }).removeAllListeners) {
        (window.electronAPI as unknown as { removeAllListeners: (event: string) => void }).removeAllListeners('connection-status-change')
      }
                                  ...
                                  <Card>
                                    <CardContent className="p-8 text-center">
                                      ...
                                      <p className="text-gray-500">
                                        {isLoading
                                          ? 'Loading messages...'
                                          : 'No messages to approve. Waiting for WebSocket messages...'}
                                      </p>
                                    </CardContent>
                                  </Card>
                                )
