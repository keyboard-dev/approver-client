 ...
 *
 * Migration Status:
 * ✅ Message approval screens migrated to /messages/:messageId route
...
export const AppContent: React.FC = () => {
  ...
  // Get code block for code response approval messages
  const getCodeBlock = useCallback((msg: Message) => {
    switch (msg.title) {
      case 'code response approval': {
        if (!msg.codespaceResponse) return null

        const { codespaceResponse } = msg
        const { data: codespaceResponseData } = codespaceResponse
        if (!codespaceResponseData) return null
  ...
  const getMessageScreen = () => {
    ...
    if (currentMessage) {
      switch (currentMessage.title) {
        ...
        case 'code response approval':
        default:
          // Default message detail view (code response approval and others)
          return (
