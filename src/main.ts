...
// Import types for REST API
interface RestAPIServerInterface {
  start: () => Promise<void>
  stop: () => Promise<void>
  getPort: () => number
  getApp: () => any // eslint-disable-line @typescript-eslint/no-explicit-any -- Express Application type not available
}
...
class MenuBarNotificationApp {
  ...
  private async requestNotificationPermissions(): Promise<void> {
    try {
      ...
      if (Notification.isSupported()) {
        // Notifications are supported, no additional action needed for basic usage
      }
      else {
  ...
  private setupIPC(): void {
    ...
    ipcMain.handle('get-auth-status', (): { authenticated: boolean, user?: AuthTokens['user'] } => {
      return {
    ...
    ipcMain.handle('get-provider-auth-status', async (): Promise<Record<string, {
    ...
    ipcMain.handle('get-oauth-storage-info', (): ReturnType<OAuthTokenStorage['getStorageInfo']> => {
